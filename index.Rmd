...
<h3>Computing Linear Regressions</h3>
```{r}
...
<h3>Linear Regression for Italy</h3>
```{r}
...
As we can see, the data for Italy has an intercept of approximately $\beta_0 = 50.5$ and an `lrscale` coefficient of $\beta_1 = 0.294$. In a linear regression, the intercept is used to indicate the expected value of the dependent variable `lrscale` at `agea` = 0. Since `agea` is never null in our data set, we conclude that the intercept is not a meaningful value for our analysis. The coefficient $\beta_1 = 0.294$, however, indicates that there is a positive correlation between age and left-to-right political index in Italy, albeit small. Specifically, for each one-unit increase in `agea`, there will be an increase of 0.294 in the value of `lrscale`. Indeed, age is correlated with being more right-wing in Italy, although weakly.

The p-value associated with this regression is $p = 0.095$. If we were to choose a statistical level of significance of $\alpha = 0.05$, and $\alpha = 0.1$, this would mean that our results would be statistically (highly) significant in the context of our study. However, this would not be true if $\alpha = 0.01$.


<h3>Linear Regression for the UK</h3>
```{r}
...
As we can see, the data for the UK has an intercept of approximately $\beta_0$ = 45.45 and an `lrscale` coefficient of $\beta_1$ = 2.118.Since `agea` is never null in our data set, we conclude that this intercept is not a meaningful value for our analysis. The coefficient $\beta_1$ = 2.118, however, indicates that there is a positive correlation between age and left-to-right political index in UK, bigger than Italy, thus confirming our visual analysis. Specifically, for each one-unit increase in `agea`, there will be an increase of 2.118 in the value of `lrscale`. Indeed, age is correlated with being more right-wing in UK, quite positively.
...
<h3>Causality and confounders</h3>
...
I can can identify multiple confounders in this research. For the purpose of this project, I will focus on one, which is readily available in the data set: 
...
To accurately assess causality, we can compute a multiple regression, this time with the confounder as the of the independent variable and `agea` and `lrscale` as the dependent variables, as to verify that education level is indeed a confounder.
...
<h3>Computing Linear Regressions</h3>
```{r}
MLR_it <- lm(edulvlb ~ agea + lrscale, data = data_italy_multiple)
MLR_uk <- lm(edulvlb ~ agea + lrscale, data = data_uk_multiple)
...
It is clear that in both cases the education level has a great impact on both `agea` and `lrscale`. Thus, we can confirm it is a confounder. As a consequence, we cannot assume causality solely between age and political ideology, both other variables need to be analyzed (possibly, all confounders) to properly assess causality.

Also comment on sample sizes for both types of regressions.
