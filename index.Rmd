...
The present study attempts to assess these shifts in power, focusing on the effect that age has on one's political ideology in Europe. I first look at the overall effect that age has on political ideology, then turn to specific country examples to understand the differences between countries whose right-wing governments have more recently settled and those whose right-wing governments have been in power for a longer period of time. 
...
<h2>Possible Conclusions</h2>
 ...
 A linear regression that supports my hypothesis would have a positive coefficient between `agea` and `lrscale`. If it were negative, it would be a linear regression that rejected my hypothesis.
...
<h3>Computing visualization of Europe's age to left-to-right index relation</h3>
```{r}
data_all |>
  group_by(age_groups) |>
...
To successfully understand if there is causality between the two values in the UK and Italy, I need to run a linear regression of the values for both countries. To do that, I will first subset the data to two separate data frames and then compute their regression. This linear regression will be run between `agea` and `lrscale`, but will also be applied to the barplot (this specification is necessary because I am **not** able to compute a linear regression between `age_groups`, a categorical variable, and `lrscale`, a numeric one, but running the linear regression with `agea` stems the desired results).
...
<h3>Single Linear Regression for Italy</h3>
```{r}
varnames1 <- c("(Intercept)" = "Intercept",
               "lrscale" = "Left-to-Right Political Index")

modelsummary::modelsummary(list(`Regression for Italy`= LR_it), 
...
The p-value associated with this regression is $p = 0.095$. If we were to choose a statistical level of significance of $\alpha = 0.1$, this would mean that our results would be (marginally) statistically significant in the context of our study. However, this would not be true if $\alpha = 0.01$ or $\alpha = 0.05$,


<h3>Single Linear Regression for the UK</h3>
```{r}

modelsummary::modelsummary(list(`Regression for the UK`= LR_uk),
                           statistic = c("s.e. = {std.error}",
                                         "p = {p.value}"),
                           gof_map = c("nobs", "r.squared", "adj.r.squared"), coef_map = varnames1)
```
...
I can can identify multiple confounders in this research. For the purpose of this project, I will focus on two, which are readily available in the data set: 
...
modelsummary::modelsummary(list(`Multiple Regression for Italy`= MLR_it), 
...
modelsummary::modelsummary(list(`Multiple Regression for the United Kingdom`= MLR_uk), 
...
<ul>
<li>**Intercept (58.565)**: This shows that the average age in the UK of respondents that are very liberal, didn't finish primary education, and do not have children in their household is approximately 59 years old.</li>
<li>**Left-to-Right Political Index (1.998)**: The coefficient for `lrscale` is 1.998, and its p-value is $p<0.001$, which indicates a statistically significant positive correlation at a statistical significance level of $\alpha = 0.05$ and other statistical significance levels. This implies that there is strong evidence of an association between political ideology and age, accounting for education level and number of children in the household, in the UK.</li>
<li>**Education Level (0.001)**: The coefficient for `edulvlb` is 0.001 with a p-value of 0.067, indicating a non-statistically significant and rather weak, positive association with age at a statistical significance level of $\alpha = 0.05$. This suggests that for each additional unit increase in the education level (the higher their education is), the respondent’s age is expected to decrease by 0.001 years, holding other variables constant (thus, older people are more educated, on average, in the UK). Because this is a very weak relationship and it is **not** statistically significant, we can't assume causality.</li>
<li>**Number of Children in Household (−6.288)**: The coefficient for `hhmmb` is -6.288 with a p-value of <0.001, suggesting a significant negative relationship with age. This means that for each additional child in the household, the respondent’s age is expected to decrease by approximately 6.288 years, holding other factors constant (thus, younger people have more children in the UK).</li>
</ul>
...
More analysis is needed, perhaps focusing on all possible confounders. However, this can prove difficult as political ideology is extremely difficult to comprehensively analyze causally, if not impossible. If provided with the needed resources, I could expand my project as to include (1) more editions of the ESS, and (2) include as many confounders as possible in my research.
...
<ul>
<li>In the simple regression, the sample size for Italy is _1928_. In the multiple regression, it is _1925_.</li>
<li>In the simple regression, the sample size for the UK is _1070_. In the multiple regression, it is _1063_.</li>
</ul>


This indicates that there is quite a large difference between the two samples. Thus, the UK data might not be as accurate as the one from Italy.
...
_This project has been completed on Wednesday, December 13th. It is the final assignment of the Harvard University undergraduate course **GOV 50: Data Science for the Social Sciences** for the Fall 2023 Semester._
